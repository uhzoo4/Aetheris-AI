...
function pickBestVideoSource(videoEl) {
  ...
  const prefersWebm = vtest.canPlayType('video/webm; codecs="vp9"') === "probably" || vtest.canPlayType("video/webm") === "probably";
  ...
  const saveData = (conn == null ? void 0 : conn.saveData) === true;
  const slow = saveData || ["slow-2g", "2g", "3g"].includes((conn == null ? void 0 : conn.effectiveType) || "");
  if (slow) wanted = Math.min(wanted, 480);
  variants.sort((a, b) => b.h - a.h);
  const choice = variants.find((v) => v.h <= wanted) || variants[variants.length - 1];
...
function applyBestSource(videoEl) {
  ...
  videoEl.load();
  const p = (_a = videoEl.play) == null ? void 0 : _a.call(videoEl);
...
function initSmartVideos() {
  ...
  const conn = navigator.connection || navigator.webkitConnection || navigator.mozConnection;
  if (conn) {
    (_a = conn.addEventListener) == null ? void 0 : _a.call(conn, "change", () => {
      document.querySelectorAll("video[data-variants]").forEach(applyBestSource);
    });
  }
  let rid;
...
var _config = {
  ...
  overwrite: false,
  delay: 0
}, _suppressOverwrites$1, _reverting$1, _context$4, _bigNum$1 = 1e8, _tinyNum = 1 / _bigNum$1, _2PI = Math.PI * 2, _HALF_PI = _2PI / 4, _gsID = 0, _sqrt = Math.sqrt, _cos = Math.cos, _sin = Math.sin, _isString$1 = function _isString2(value) {
...
var _win$3, _doc$3, _docElement, _pluginInitted, _tempDiv, _recentSetterPlugin, _reverting, _windowExists$2 = function _windowExists22() {
  ...
  if ((toPercent || curUnit === "%") && (_transformProps[property] || ~property.indexOf("adius"))) {
    ...
    return _round$2(toPercent ? curValue / px2 * amount : curValue / 100 * px2);
  }
  ...
  return _round$2(toPixels ? px2 * curValue / amount : px2 && curValue ? amount / px2 * curValue : 0);
...
const TOGGLE_SELECTOR = "[data-fls-menu]";
const MENU_SELECTOR = ".menu__body";
const LIST_ITEMS_SELECTOR = ".menu__list .menu__item";
const INFO_CONTAINER_SELECTOR = ".menu__info";
const ATTR_OPEN = "data-fls-menu-open";
const ATTR_ANIM = "data-fls-menu-animating";
...
function menuInit() {
  ...
  if (!btn || !menu) return;
  const getNodes = () => {
    const infoContainer = menu.querySelector(INFO_CONTAINER_SELECTOR) || // фолбек: раптом .menu__info не в menu (рідко, але)
    document.querySelector(INFO_CONTAINER_SELECTOR);
    let sections = [];
    if (infoContainer) {
      sections = Array.from(infoContainer.querySelectorAll(".info__section"));
      if (!sections.length) sections = Array.from(infoContainer.children);
    }
    return {
      items: Array.from(menu.querySelectorAll(LIST_ITEMS_SELECTOR)),
      infoContainer,
      sections
    };
  };
  const prepInitial = ({ items, infoContainer, sections }) => {
    if (items.length)
      gsapWithCSS.set(items, { opacity: 0, y: 12, pointerEvents: "none" });
    if (infoContainer) gsapWithCSS.set(infoContainer, { opacity: 0, y: 8 });
    if (sections.length)
      gsapWithCSS.set(sections, { opacity: 0, y: 16, pointerEvents: "none" });
  };
  const animateIn = () => {
    const nodes = getNodes();
    prepInitial(nodes);
    isAnimating = true;
    ROOT.setAttribute(ATTR_ANIM, "opening");
    const tl = gsapWithCSS.timeline({
      defaults: { ease: "power3.out" },
      onComplete() {
        ROOT.removeAttribute(ATTR_ANIM);
        isAnimating = false;
      }
    });
    ...
    const parallelNodes = [];
    if (nodes.sections.length) parallelNodes.push(nodes.sections);
    if (nodes.items.length) parallelNodes.push(nodes.items);
    if (parallelNodes.length) {
      tl.to(
        parallelNodes,
        {
          ...
          stagger: 0.06,
          onStart: () => {
            if (nodes.sections.length) gsapWithCSS.set(nodes.sections, { pointerEvents: "auto" });
            if (nodes.items.length) gsapWithCSS.set(nodes.items, { pointerEvents: "auto" });
          }
        },
        nodes.infoContainer ? "<+0.05" : 0
      );
    }
  };
  const animateOut = () => {
    const nodes = getNodes();
    isAnimating = true;
    ROOT.setAttribute(ATTR_ANIM, "closing");
    const tl = gsapWithCSS.timeline({
      defaults: { ease: "power2.in" },
      onComplete() {
        if (nodes.items.length)
          gsapWithCSS.set(nodes.items, { opacity: 0, y: 12, pointerEvents: "none" });
        if (nodes.sections.length)
          gsapWithCSS.set(nodes.sections, { opacity: 0, y: 16, pointerEvents: "none" });
        if (nodes.infoContainer)
          gsapWithCSS.set(nodes.infoContainer, { opacity: 0, y: 8 });
    ...
    if (nodes.items.length) {
      tl.to(
        nodes.items,
        {
          ...
          duration: 0.3,
          stagger: 0.06
          // знизу-вгору: stagger: { each: 0.06, from: 'end' }
        },
        0
      );
    }
    if (nodes.sections.length) {
      tl.to(
        nodes.sections,
        {
          ...
          duration: 0.35,
          stagger: 0.08
          // знизу-вгору: stagger: { each: 0.08, from: 'end' }
        },
        nodes.items.length ? ">-0.05" : 0
      );
    }
    if (nodes.infoContainer) {
      tl.to(
  ...
  btn.addEventListener("click", (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (!bodyLockStatus || isAnimating) return;
    ROOT.hasAttribute(ATTR_OPEN) ? closeMenu() : openMenu();
  });
  if (ROOT.hasAttribute(ATTR_OPEN)) {
    const nodes = getNodes();
    if (nodes.infoContainer) gsapWithCSS.set(nodes.infoContainer, { opacity: 1, y: 0 });
    if (nodes.sections.length)
      gsapWithCSS.set(nodes.sections, { opacity: 1, y: 0, pointerEvents: "auto" });
    if (nodes.items.length)
      gsapWithCSS.set(nodes.items, { opacity: 1, y: 0, pointerEvents: "auto" });
  }
}
document.querySelector(TOGGLE_SELECTOR) ? window.addEventListener("load", menuInit) : null;
...
function magnetEffect() {
  ...
  function parseOpts(root) {
    let opts = { max: 18, scale: 1.04, rotate: 4, ease: 0.15, target: null };
    const raw = root.getAttribute("data-fls-magnet");
    if (raw && raw !== "") {
      raw.split(";").forEach((pair) => {
        ...
        if (!k || !v) return;
        if (k === "max") opts.max = +v;
        else if (k === "scale") opts.scale = +v;
        else if (k === "rotate") opts.rotate = +v;
        else if (k === "ease") opts.ease = +v;
        else if (k === "target") opts.target = v;
      });
    }
    if (root.hasAttribute("data-fls-magnet-max"))
      opts.max = +root.getAttribute("data-fls-magnet-max");
    if (root.hasAttribute("data-fls-magnet-scale"))
      opts.scale = +root.getAttribute("data-fls-magnet-scale");
    if (root.hasAttribute("data-fls-magnet-rotate"))
      opts.rotate = +root.getAttribute("data-fls-magnet-rotate");
    if (root.hasAttribute("data-fls-magnet-ease"))
      opts.ease = +root.getAttribute("data-fls-magnet-ease");
    if (root.hasAttribute("data-fls-magnet-target"))
      opts.target = root.getAttribute("data-fls-magnet-target");
    opts.max = isFinite(opts.max) ? opts.max : 18;
    opts.scale = isFinite(opts.scale) ? opts.scale : 1.04;
    opts.rotate = isFinite(opts.rotate) ? opts.rotate : 4;
    opts.ease = Math.min(
      0.35,
      Math.max(0.06, isFinite(opts.ease) ? opts.ease : 0.15)
    );
  ...
  function ensureState(root) {
    ...
    const { max, scale, rotate, ease, target } = parseOpts(root);
    const node = target ? root.querySelector(target) : root;
    if (!node) return null;
    const s = {
      root,
      node,
  ...
  function tick(s) {
    ...
    s.r += er * s.ease;
    s.sc += es * s.ease;
    s.node.style.transform = `translate3d(${s.x.toFixed(2)}px, ${s.y.toFixed(2)}px, 0) ` + (s.rotate ? `rotate(${s.r.toFixed(2)}deg) ` : "") + `scale(${s.sc.toFixed(3)})`;
    const settled = !s.hovering && Math.abs(ex) < 0.1 && Math.abs(ey) < 0.1 && Math.abs(er) < 0.1 && Math.abs(es) < 1e-3;
    if (settled) {
  ...
  function updateToPointer(s, clientX, clientY) {
    ...
    const m = Math.min(dist, s.max);
    const ux = dx / dist;
    const uy = dy / dist;
    s.tx = ux * m;
    s.ty = uy * m;
    if (s.rotate) {
      const nx = Math.max(-1, Math.min(1, dx / (rect.width / 2 || 1)));
      s.tr = nx * s.rotate;
    } else {
      s.tr = 0;
    }
  ...
  document.addEventListener("pointerover", (e) => {
    const root = e.target.closest("[data-fls-magnet]");
    if (!root) return;
    const s = ensureState(root);
    if (!s) return;
    if (reduceMotion) {
      s.node.style.transition = "transform .2s ease";
      s.node.style.transform = `scale(${s.scale})`;
      return;
    }
    s.hovering = true;
    updateToPointer(s, e.clientX, e.clientY);
    start(s);
  });
  document.addEventListener("pointermove", (e) => {
    const root = e.target.closest("[data-fls-magnet]");
    if (!root) return;
    const s = states.get(root) || ensureState(root);
    if (!s || !s.hovering || reduceMotion) return;
    updateToPointer(s, e.clientX, e.clientY);
  });
  document.addEventListener("pointerout", (e) => {
    ...
    if (!s) return;
    if (reduceMotion) {
      s.node.style.transform = "scale(1)";
      return;
    }
    s.hovering = false;
    s.tx = 0;
...
document.querySelector("[data-fls-magnet]") ? window.addEventListener("load", magnetEffect) : null;
function customCursor() {
  const hasCursor = document.querySelector("[data-fls-cursor]");
  if (hasCursor && !isMobile.any()) {
    let mouseActions = function(e) {
      if (inBlockedZone(e.target)) {
        hideCursor();
        return;
      } else {
        showCursor();
      ...
      if (e.type === "mousemove") {
        ...
        if (e.target.closest(
          'button, a, input, label, [role="button"], [tabindex]'
        ) || cur && cur !== "none" && cur !== "default") {
    ...
    cursor.className = "fls-cursor is-hidden";
    cursor.innerHTML = `<span class="fls-cursor__pointer"></span>${isShadowTrue ? '<span class="fls-cursor__shadow"></span>' : ""}`;
    document.body.append(cursor);
    const cursorPointer = cursor.querySelector(".fls-cursor__pointer");
    const cursorShadow = cursor.querySelector(".fls-cursor__shadow") || null;
    const cursorPointerStyle = {
      width: cursorPointer.offsetWidth,
      height: cursorPointer.offsetHeight
    };
    const cursorShadowStyle = cursorShadow ? {
      width: cursorShadow.offsetWidth,
      height: cursorShadow.offsetHeight
    } : null;
    const BLOCK_SELECTOR = "[data-cursor-skip]";
    const inBlockedZone = (el) => {
      var _a;
      return !!((_a = el == null ? void 0 : el.closest) == null ? void 0 : _a.call(el, BLOCK_SELECTOR));
    };
    let hidden = true;
    const hideCursor = () => {
      if (!hidden) {
    ...
    const onPointerOver = (e) => {
      if (inBlockedZone(e.target)) hideCursor();
    };
    const onPointerOut = (e) => {
      if (inBlockedZone(e.target) && !inBlockedZone(e.relatedTarget))
        showCursor();
    };
    ...
    const showOnFirstMove = (e) => {
      showCursor();
      mouseActions(e);
      window.removeEventListener("mousemove", showOnFirstMove);
    };
    window.addEventListener("mousemove", showOnFirstMove);
  }
}
 ...
 * @author: Jack Doyle, jack@greensock.com
*/
var gsap$1, _coreInitted$1, _win, _doc, _docEl, _body, _toArray$1, _clamp, ScrollTrigger, _mainInstance, _expo, _getVelocityProp2, _inputObserver2, _context$1, _onResizeDelayedCall, _windowExists3 = function _windowExists4() {
  return typeof window !== "undefined";
...
gsapWithCSS.registerPlugin(ScrollTrigger$1, ScrollSmoother, SplitText);
function animateTitlesOnScroll() {
  document.querySelectorAll(".shop-up").forEach((el) => {
    let split;
      ...
      onEnter: () => {
        ...
        gsapWithCSS.from(split.lines, {
          ...
          onComplete: () => {
            split.revert();
            split = null;
            gsapWithCSS.delayedCall(0, () => ScrollTrigger$1.refresh());
            el.style.visibility = "visible";
          }
        });
      }
...
function revealWavyLinesOnScroll() {
  document.querySelectorAll(".wavy-block").forEach((block) => {
    const paths = block.querySelectorAll(".wavy-block__path");
    ...
    gsapWithCSS.to(paths, {
      ...
      scrollTrigger: {
        trigger: block,
        // draw when the whole wavy-block appears
        start: "top 90%",
...
window.addEventListener("load", () => {
  var _a;
  if ((_a = document.fonts) == null ? void 0 : _a.ready) {
    document.fonts.ready.then(revealWavyLinesOnScroll);
  } else {
    revealWavyLinesOnScroll();
  }
});
function ensureUnderlinePathUnderInfo(card) {
  const info = card.querySelector(".column-item__info");
  if (!info) return null;
  let svg = card.querySelector(":scope > .ci-underline");
  if (!svg) {
    svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.classList.add("ci-underline");
    svg.setAttribute("viewBox", "0 0 100 2");
    svg.setAttribute("preserveAspectRatio", "none");
    const path = document.createElementNS("http://www.w3.org/2000/svg", "path");
    path.setAttribute("d", "M100 1 L0 1");
    path.setAttribute("fill", "none");
    path.setAttribute("stroke", "currentColor");
    path.setAttribute("stroke-width", "2");
    svg.appendChild(path);
    info.insertAdjacentElement("afterend", svg);
  }
  return svg.querySelector("path");
}
function initColumnInfoRevealRTL() {
  gsapWithCSS.utils.toArray(".column-item__body").forEach((card) => {
    if (card.__infoInitedRTL) return;
    card.__infoInitedRTL = true;
    const info = card.querySelector(".column-item__info");
    const workType = info == null ? void 0 : info.querySelector(".column-item__work-type");
    const dateEl = info == null ? void 0 : info.querySelector(".column-item__date");
    const path = ensureUnderlinePathUnderInfo(card);
    if (!info || !workType || !dateEl || !path) return;
    gsapWithCSS.set([workType, dateEl], { autoAlpha: 0 });
    ...
    tl.to(path, { strokeDashoffset: 0, duration: TOTAL }, 0);
    tl.fromTo(workType, { autoAlpha: 0 }, { autoAlpha: 1, duration: TOTAL }, 0);
    tl.fromTo(
      dateEl,
      { autoAlpha: 0 },
      { autoAlpha: 1, duration: TOTAL - GAP },
      GAP
    );
  });
}
(function fadeInButtonsOnScroll() {
  var _a;
  const BTN_SELECTOR = ".column-item__body--btn .column-item__button";
  function hideButtons(scope = document) {
    const btns = scope.querySelectorAll(BTN_SELECTOR);
    btns.forEach((btn) => {
      if (btn.__fadeInit) return;
      gsapWithCSS.set(btn, { autoAlpha: 0 });
      btn.__fadeInit = true;
    });
  }
  function createBatch() {
    ScrollTrigger$1.batch(BTN_SELECTOR, {
      start: "top 90%",
      once: true,
      onEnter: (els) => {
        gsapWithCSS.to(els, {
  ...
  const observer = new MutationObserver((muts) => {
    ...
    muts.forEach((m) => {
      m.addedNodes.forEach((node) => {
        ...
        if ((_a2 = node.matches) == null ? void 0 : _a2.call(node, BTN_SELECTOR)) {
          hideButtons(node.parentNode || document);
          added = true;
        } else {
          const found = (_b = node.querySelectorAll) == null ? void 0 : _b.call(node, BTN_SELECTOR);
          if (found == null ? void 0 : found.length) {
            hideButtons(node);
            added = true;
          }
        }
    ...
    if (added) gsapWithCSS.delayedCall(0, () => ScrollTrigger$1.refresh());
  });
  const run = () => {
    hideButtons();
    createBatch();
  ...
  if ((_a = document.fonts) == null ? void 0 : _a.ready) {
    document.fonts.ready.then(() => ScrollTrigger$1.refresh());
  }
})();
(function boot() {
  const run = () => {
    initColumnInfoRevealRTL();
    gsapWithCSS.delayedCall(0, () => ScrollTrigger$1.refresh());
  };
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", run, { once: true });
...
})();
{
  ...
  if (pl && sc && sc.contains(pl)) document.body.prepend(pl);
}
const FIRST_DELAY = 0.25;
const ROW_DUR = 0.8;
const ROW_STAGGER = 0.3;
const WAIT_FADE_IN = 0.5;
const WORD_FADE_DUR = 0.4;
const WORD_STAGGER = 0.07;
const CURTAIN_DUR = 0.7;
function wrapWordsInPlace(root, excludeSelector) {
  const words = [];
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (excludeSelector && ((_a = node.parentElement) == null ? void 0 : _a.closest(excludeSelector)))
        return NodeFilter.FILTER_REJECT;
...
function startPreloader() {
  return new Promise((resolve) => {
    const run = () => {
      ...
      const topEl = document.querySelector(".preloader__top");
      const lines = Array.from(
        document.querySelectorAll(".preloader__text .preloader__line")
      );
      const waitEl = document.querySelector(".preloader__wait");
      const countEl = document.querySelector(".js-count");
      const nowWrap = document.querySelector(".js-now");
      const nowReg = nowWrap == null ? void 0 : nowWrap.querySelector(".preloader__now-regular");
      const nowIta = nowWrap == null ? void 0 : nowWrap.querySelector(".preloader__now-italic");
      if (!preloader || !topEl || lines.length < 2) {
        html.setAttribute("data-app-ready", "");
        html.classList.remove("preload-lock");
        (_a = document.querySelector(".preloader")) == null ? void 0 : _a.remove();
        resolve();
        return;
      }
      lines.forEach((line) => {
        if (!line.querySelector(".preloader__line-inner")) {
          const inner = document.createElement("span");
          inner.className = "preloader__line-inner";
          while (line.firstChild) inner.appendChild(line.firstChild);
          line.appendChild(inner);
        }
      });
      const inners = Array.from(
        document.querySelectorAll(".preloader__line-inner")
      );
      gsapWithCSS.set([topEl, ...inners], { yPercent: 120, opacity: 0 });
      if (waitEl) gsapWithCSS.set(waitEl, { opacity: 0 });
      const rowsCount = 1 + inners.length;
      const totalDur = FIRST_DELAY + ROW_DUR + ROW_STAGGER * (rowsCount - 1) + WAIT_FADE_IN;
      const tl = gsapWithCSS.timeline({ defaults: { overwrite: "auto" } });
      if (countEl) {
        const counter = { v: 0 };
        tl.to(
          counter,
      ...
      tl.to(
        [topEl, inners[0], inners[1]],
        {
          yPercent: 0,
          opacity: 1,
          duration: ROW_DUR,
          ease: "expo.out",
          stagger: { each: ROW_STAGGER }
        },
        FIRST_DELAY
      );
      if (waitEl) {
        tl.to(
          waitEl,
          { opacity: 1, duration: WAIT_FADE_IN, ease: "power1.out" },
          ">+0.06"
        );
      }
      if (nowReg && nowIta) {
        gsapWithCSS.set(nowReg, { opacity: 1, visibility: "visible" });
        gsapWithCSS.set(nowIta, { opacity: 0, visibility: "hidden" });
        const BLINK_FADE = 0.28, BLINK_HOLD = 0.26;
        gsapWithCSS.timeline({ repeat: -1 }).to(nowReg, {
          opacity: 0,
          duration: BLINK_FADE,
          ease: "power1.inOut"
        }).set(nowReg, { visibility: "hidden" }).set(nowIta, { visibility: "visible" }).to(nowIta, { opacity: 1, duration: 1e-3 }).to({}, { duration: BLINK_HOLD }).to(nowIta, {
          opacity: 0,
          duration: BLINK_FADE,
          ease: "power1.inOut"
        }).set(nowIta, { visibility: "hidden" }).set(nowReg, { visibility: "visible" }).to(nowReg, { opacity: 1, duration: 1e-3 }).to({}, { duration: BLINK_HOLD });
      }
      const wordsPool = [];
      const yourEl = document.querySelector(".preloader__your");
      if (yourEl) wordsPool.push(...wrapWordsInPlace(yourEl));
      const countBlock = document.querySelector(".preloader__count");
      if (countBlock) wordsPool.push(countBlock);
      inners.forEach((inner) => {
        const nowInLine = inner.querySelector(".js-now");
        if (nowInLine) {
          inner.childNodes.forEach((node) => {
            if (node.nodeType === 3) {
              ...
              wordsPool.push(...wrapWordsInPlace(temp, ".js-now"));
              while (temp.firstChild)
                temp.parentNode.insertBefore(temp.firstChild, temp);
              temp.remove();
            } else if (node.nodeType === 1 && !node.classList.contains("js-now")) {
              wordsPool.push(...wrapWordsInPlace(node, ".js-now"));
            }
          });
          wordsPool.push(nowInLine);
        } else {
          wordsPool.push(...wrapWordsInPlace(inner, ".js-now"));
        }
      });
      tl.call(
        () => {
          const shuffled = shuffle(wordsPool);
          gsapWithCSS.to(shuffled, {
            opacity: 0,
            duration: WORD_FADE_DUR,
            ease: "power1.out",
            stagger: { each: WORD_STAGGER, from: "random" }
          });
          if (waitEl)
            gsapWithCSS.to(waitEl, {
              opacity: 0,
              duration: WORD_FADE_DUR,
              ease: "power1.out"
            });
          const totalWordsTime = WORD_FADE_DUR + WORD_STAGGER * shuffled.length + 0.05;
          gsapWithCSS.to(preloader, {
            yPercent: -100,
            duration: CURTAIN_DUR,
            ease: "power2.in",
            delay: totalWordsTime,
            onComplete: () => {
              html.setAttribute("data-app-ready", "");
              html.classList.remove("preload-lock");
              preloader.remove();
...
(async () => {
  ...
  requestAnimationFrame(() => ScrollTrigger$1.refresh());
})();
const isTouchDevice = window.matchMedia && window.matchMedia("(pointer: coarse)").matches || "ontouchstart" in window || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0;
const px = (n) => `${n}px`;
function typedSwapEffect() {
  var _a;
  const ATTR = "data-fls-typed";
  const cache = /* @__PURE__ */ new WeakMap();
  const splitGraphemes = (str) => {
    var _a2;
    if ((_a2 = window.Intl) == null ? void 0 : _a2.Segmenter) {
      try {
  ...
  const parseOpts = (root) => {
    ...
    if (raw) {
      raw.split(";").forEach((p) => {
        ...
        const k = k0.trim();
        const v = v0.trim();
        if (k === "duration") def.duration = Number(v);
        else if (k === "stagger") def.stagger = Number(v);
        else if (k === "ease") def.ease = v;
        else if (k === "direction") def.direction = v === "rtl" ? "rtl" : "ltr";
        else if (k === "skew") def.skew = Number(v);
      });
    }
    const pick = (n) => root.hasAttribute(`${ATTR}-${n}`) ? root.getAttribute(`${ATTR}-${n}`) : null;
    def.duration = Number(pick("duration") ?? def.duration);
    def.stagger = Number(pick("stagger") ?? def.stagger);
    def.ease = pick("ease") ?? def.ease;
    def.direction = pick("direction") ?? def.direction;
    def.skew = Number(pick("skew") ?? def.skew);
    return def;
  };
  function ensure(root) {
    if (cache.has(root)) return cache.get(root);
    root.classList.add("tsw-pending");
    const opt = parseOpts(root);
    root.style.setProperty("--tsw-dur", `${opt.duration}s`);
    root.style.setProperty("--tsw-ease", opt.ease);
    let base = root.querySelector(".tsw__base");
    let alt = root.querySelector(".tsw__alt");
    if (!base) {
    ...
    if (!alt) {
      ...
      alt.textContent = base.textContent;
      root.appendChild(alt);
    }
    alt.style.transform = opt.skew ? `skewX(${opt.skew}deg)` : "";
    if (!base.querySelector(".char") && !alt.querySelector(".char")) {
      const chars = splitGraphemes(base.textContent);
      base.textContent = "";
      alt.textContent = "";
      const ordered = opt.direction === "rtl" ? [...chars].reverse() : chars;
      ordered.forEach((_, i) => {
        const idx = opt.direction === "rtl" ? chars.length - 1 - i : i;
        const ch = chars[idx] === " " ? " " : chars[idx];
        const delay = (opt.stagger * i).toFixed(3) + "s";
    ...
    root.classList.remove("tsw-pending");
    root.classList.add("is-ready");
    const st = { primed: false, opt };
    cache.set(root, st);
    return st;
  }
  const initAll = () => {
    document.querySelectorAll(`[${ATTR}]`).forEach(ensure);
  };
  if ((_a = document.fonts) == null ? void 0 : _a.ready) {
  ...
  document.addEventListener("pointerover", (e) => {
    const root = e.target.closest(`[${ATTR}]`);
    if (!root) return;
    ...
    if (!st.primed) {
      ...
      requestAnimationFrame(
        () => requestAnimationFrame(() => root.classList.add("is-hover"))
      );
    } else {
...
window.addEventListener("load", typedSwapEffect);
(function flagHover() {
  ...
  const srcByKey = {};
  imgs.forEach((el) => {
    srcByKey[(el.dataset.image || "").toLowerCase()] = el.getAttribute("src");
  });
  const FLAG = {
    baseWidth: 340,
    easing: 0.12,
  ...
  const vertexShader = `
	...
	uniform float uTime, uFreqX, uFreqY, uAmp;
	varying vec2  vUv;
	vec3 deform(vec3 p, vec2 uv, vec2 off){
	  float f = 0.035;
	  p.x += sin(uv.y * uFreqY + uTime * 2.0) * off.x * uAmp * f;
	  p.y += sin(uv.x * uFreqX + uTime * 1.5) * off.y * uAmp * f;
	  return p;
	}
	void main(){
	  vUv = uv;
	  vec3 pos = deform(position, uv, uOffset);
  ...
  const geometry = new THREE.PlaneGeometry(1, 1, FLAG.segments, FLAG.segments);
  const uniforms = {
    uTexture: { value: null },
    uOffset: { value: new THREE.Vector2(0, 0) },
    uAlpha: { value: 0 },
    uTime: { value: 0 },
    uFreqX: { value: FLAG.freqX },
    uFreqY: { value: FLAG.freqY },
    uAmp: { value: FLAG.amp }
  };
  ...
  function loadTextureForKey(key, url) {
    return new Promise((resolve) => {
      if (!url) {
        texByKey[key] = null;
        return resolve();
      ...
      loader.load(
        ...
        (tex) => {
          ...
          tex.needsUpdate = true;
          texByKey[key] = { tex, w: tex.image.width, h: tex.image.height };
          resolve();
        },
        void 0,
        () => resolve()
  ...
  async function preloadTextures() {
    const jobs = Object.entries(srcByKey).map(
      ([k, u]) => loadTextureForKey(k, u)
    );
  ...
  const state = {
    ...
    current: null
    // current word element under cursor
  };
  const baseWidth = () => FLAG.baseWidth * (innerWidth / 1440);
  function setPlaneSizeForKey(key) {
    const info = texByKey[key];
    const w = baseWidth();
    const ratio = info ? info.h / info.w : 1;
    mesh.scale.set(w, w * ratio, 1);
  }
  function showCanvas() {
    if (!state.isVisible) {
      canvas.classList.add("flag-canvas--visible");
  ...
  function setGoalFromMouse(e) {
    state.goal.set(e.clientX - W / 2, H - e.clientY - H / 2);
  }
  addEventListener("mousemove", (e) => {
    const x = (e.clientX / W - 0.5) * 2;
    const y = (e.clientY / H - 0.5) * 2;
    state.parallax.set(
      x * FLAG.baseWidth * FLAG.parallaxK,
      -y * FLAG.baseWidth * FLAG.parallaxK
    );
  });
  function bindWordHandlersDelegated() {
    document.addEventListener("pointerover", (e) => {
      var _a;
      const el = e.target.closest(".hero__word--flag-target[data-word]");
      if (!el || el === state.current) return;
      state.current = el;
      const key = (el.dataset.word || "").toLowerCase();
      uniforms.uTexture.value = ((_a = texByKey[key]) == null ? void 0 : _a.tex) || fallbackTex;
      setPlaneSizeForKey(key);
      setGoalFromMouse(e);
      state.pos.copy(state.goal);
      state.last.copy(state.pos);
      showCanvas();
    });
    document.addEventListener("pointermove", (e) => {
      if (!state.current) return;
      const el = e.target.closest(".hero__word--flag-target");
      if (!el || el !== state.current) return;
      setGoalFromMouse(e);
    });
    document.addEventListener("pointerout", (e) => {
      ...
      if (related && state.current.contains(related)) return;
      const maybeSame = related && ((_a = related.closest) == null ? void 0 : _a.call(related, ".hero__word--flag-target"));
      if (maybeSame === state.current) return;
      state.current = null;
      hideCanvas();
    });
  }
  function loop() {
    uniforms.uTime.value = gsapWithCSS.ticker.time;
    const k = state.targetAlpha > uniforms.uAlpha.value ? FLAG.fadeIn : FLAG.fadeOut;
    uniforms.uAlpha.value += (state.targetAlpha - uniforms.uAlpha.value) * k;
    if (state.isVisible && state.targetAlpha === 0 && uniforms.uAlpha.value < 0.02) {
      canvas.classList.remove("flag-canvas--visible");
      state.isVisible = false;
    }
    state.pos.lerp(state.goal, FLAG.easing);
    const dx = state.pos.x - state.last.x;
    const dy = state.pos.y - state.last.y;
    uniforms.uOffset.value.set(dx * FLAG.offsetK, dy * FLAG.offsetK);
    mesh.position.set(
      state.pos.x + state.parallax.x,
      state.pos.y + state.parallax.y,
      0
    );
    state.last.copy(state.pos);
  ...
  ScrollTrigger$1.addEventListener("refreshInit", onResize);
  (async () => {
    await preloadTextures();
    bindWordHandlersDelegated();
    gsapWithCSS.ticker.add(loop);
  })();
})();
window.addEventListener("DOMContentLoaded", () => {
  const SMOOTH = 0.08;
  const CLAMP_PAD_MIN = 8;
  const HOME_X_FALLBACK = "50%";
  const HOME_Y_FALLBACK = "0%";
  const parseVar = (val, size) => {
    if (!val) return null;
    val = val.trim();
  ...
  const isTouchy = window.matchMedia(
    "(hover: none) and (pointer: coarse)"
  ).matches;
  document.querySelectorAll(".video__wrapper").forEach((wrapper) => {
    const video = wrapper.querySelector(".video__obj");
    const btn = wrapper.querySelector(".video__button");
    if (!video || !btn) return;
    wrapper.classList.toggle("is-touch", isTouchy);
    let rect = wrapper.getBoundingClientRect();
    const half = () => btn.offsetWidth / 2;
    const readHome = () => {
      const cs = getComputedStyle(wrapper);
      const xVar = cs.getPropertyValue("--home-x") || HOME_X_FALLBACK;
      const yVar = cs.getPropertyValue("--home-y") || HOME_Y_FALLBACK;
      let hx = parseVar(xVar, rect.width);
      let hy = parseVar(yVar, rect.height);
      if (hx == null) hx = rect.width * 0.5;
      if (hy == null) hy = rect.height * 0.5;
      const padX = Math.max(half(), CLAMP_PAD_MIN);
      hx = Math.max(padX, Math.min(rect.width - padX, hx));
      const padYmax = Math.max(half(), CLAMP_PAD_MIN);
      hy = Math.max(0, Math.min(rect.height - padYmax, hy));
      return { x: hx, y: hy };
    };
    ...
    const loop = () => {
      ...
      setButton(pos.x, pos.y);
      const moving = Math.hypot(target.x - pos.x, target.y - pos.y) > 0.3;
      if (hovering || moving) raf = requestAnimationFrame(loop);
      else raf = null;
    };
    const clampPos = (x, y) => {
      const pad = Math.max(half(), CLAMP_PAD_MIN);
    ...
    setButton(HOME.x, HOME.y);
    if (canHover) {
      wrapper.addEventListener("pointerenter", onMove);
      wrapper.addEventListener("pointermove", onMove);
      wrapper.addEventListener("pointerleave", goHome);
    }
    let resizeTO;
    ...
    ["loadeddata", "canplay"].forEach((ev) => {
      video.addEventListener(ev, () => wrapper.classList.add("is-ready"), {
        once: true
      });
    });
    const sync = () => {
      ...
      btn.classList.toggle("is-square", playing);
      btn.setAttribute("aria-pressed", String(playing));
      btn.setAttribute("aria-label", playing ? "pause video" : "play video");
    };
    const toggle = () => {
      if (video.paused || video.ended) {
        if (video.muted == null) video.muted = true;
        const p = video.play();
    ...
    ["play", "pause", "ended"].forEach((ev) => video.addEventListener(ev, sync));
    sync();
  });
});
(function initScrollIndicator() {
  const roots = document.querySelectorAll("[data-scroll-indicator]");
  if (!roots.length) return;
  const _clamp3 = (v, a, b) => Math.min(b, Math.max(a, v));
  let lastY = 0;
  let didInit = false;
  ScrollTrigger$1.create({
    start: 0,
    ...
    onUpdate: (st) => {
      const y = st.scroll();
      const dy = didInit ? y - lastY : 0;
      lastY = y;
      didInit = true;
      roots.forEach((root) => {
        var _a;
        return (_a = root.__si_onScroll) == null ? void 0 : _a.call(root, y, dy);
      });
    }
  });
  roots.forEach((root) => {
    const items = root.querySelectorAll("[data-scroll-indicator-text]");
    if (items.length < 2) return;
    items[0].style.transform = "translate(0%, 0%) translate3d(0,0,0)";
    items[1].style.transform = "translate(0%, -100%) translate3d(0,0,0)";
    const speed = Number(root.dataset.siSpeed) || 60;
    const dir = (root.dataset.siDir || "down").toLowerCase();
    const sign = dir === "up" || dir === "-1" ? -1 : 1;
    const mode = (root.dataset.siMode || "edge").toLowerCase();
    const which = (root.dataset.siWhich || "end").toLowerCase();
    const slowAmt = _clamp3(Number(root.dataset.siSlow ?? 0.8), 0, 0.95);
    const widthPct = _clamp3(Number(root.dataset.siWidth ?? 18), 1, 100);
    const TOP_SHOW_THRESHOLD = 10;
    let hidden = false;
    root.__si_onScroll = (y, dy) => {
      if (y > TOP_SHOW_THRESHOLD && dy > 0.5 && !hidden) {
        hidden = true;
        root.classList.add("scroll-indicator--fadeout");
      }
      if (y <= TOP_SHOW_THRESHOLD && hidden) {
        hidden = false;
        root.classList.remove("scroll-indicator--fadeout");
      }
    };
    const modAt = (p) => {
      if (mode === "edge") {
        const distToEnd = 100 - p;
        const distToStart = p;
        let d;
        if (which === "both") d = Math.min(distToStart, distToEnd);
        else if (which === "start") d = distToStart;
        else d = distToEnd;
        if (d >= widthPct) return 1;
        const t = 1 - d / widthPct;
        const w = (1 - Math.cos(Math.PI * t)) / 2;
        return 1 - slowAmt * w;
      } else {
        const d = Math.abs(p - 50);
        const sigma = widthPct / 2.5;
    ...
    let pos = 0;
    const loop = () => {
      const dt = gsapWithCSS.ticker.deltaRatio() / 60;
      let p = pos;
      if (p >= 100) p -= 100;
      if (p < 0) p += 100;
      const mod = _clamp3(modAt(p), 0.05, 1);
      pos += sign * speed * mod * dt;
      if (pos >= 100) pos -= 100;
      if (pos < 0) pos += 100;
      items[0].style.transform = `translate(0%, ${pos}%) translate3d(0,0,0)`;
      items[1].style.transform = `translate(0%, ${pos - 100}%) translate3d(0,0,0)`;
    };
    gsapWithCSS.ticker.add(loop);
  });
  ScrollTrigger$1.addEventListener("refreshInit", () => {
    lastY = 0;
    didInit = false;
  });
})();
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!window.matchMedia("(min-width:1068px)").matches) return;
  ...
  const createProgram = (gl, vsSource, fsSource) => {
    ...
    if (!gl.getProgramParameter(pr, gl.LINK_STATUS)) {
      throw new Error("Program link error: " + gl.getProgramInfoLog(pr));
    }
    return pr;
  };
  ...
  const setCanvasSize = (canvas, w, h) => {
  ...
  const coverScale = (cw, ch, iw, ih) => {
    const rc = cw / ch, ri = iw / ih;
    return rc > ri ? [ri / rc, 1] : [1, rc / ri];
  };
  const VS = `
...
`;
  ...
  document.querySelectorAll(".column-item__images").forEach(async (wrap3) => {
    const imgs = wrap3.querySelectorAll("img");
    if (imgs.length < 2) return;
    const image1 = imgs[0].getAttribute("src");
    const image2 = imgs[1].getAttribute("src");
    const displacement = wrap3.getAttribute("data-displace") || "/assets/img/effect-image.jpg";
    ...
    const resize = () => {
      ...
      const s1 = coverScale(canvas.width, canvas.height, im1.width, im1.height);
      gl.useProgram(program);
    ...
    wrap3.addEventListener("touchend", () => {
      target = 0;
      start = performance.now();
    });
  });
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".column-item__button").forEach((btn) => {
    const txt = btn.dataset.hoverText;
    const textNode = btn.querySelector("span");
    if (txt && textNode) textNode.textContent = txt;
    if (!btn.hasAttribute("aria-label") && textNode) {
      btn.setAttribute("aria-label", textNode.textContent.trim());
    }
    const press = () => {
...
window.matchMedia && matchMedia("(pointer: coarse)").matches || "ontouchstart" in window || navigator.maxTouchPoints > 0;
const WAVY_DEFAULTS = {
  sensitivity: 0.7,
  // sensitivity to movementY: larger = stronger reaction
  strength: 1,
  // multiplier for bend strength
  maxBend: 60,
  // amplitude limit (px)
  settle: 0.035,
  // return speed to 0 in animation
  freqStep: 0.18
  // speed of "oscillation" when releasing
};
if (isTouchDevice) {
  document.querySelectorAll(".js-wavy").forEach((h) => {
    const wrap3 = h.closest(".wavy-block") || h.parentElement;
    if (wrap3) wrap3.classList.add("wavy-block--no-touch");
  });
} else {
  document.querySelectorAll(".js-wavy").forEach((h) => {
    const cfg = {
      sensitivity: parseFloat(h.dataset.wavySensitivity) || WAVY_DEFAULTS.sensitivity,
      strength: parseFloat(h.dataset.wavyStrength) || WAVY_DEFAULTS.strength,
      maxBend: parseFloat(h.dataset.wavyMax) || WAVY_DEFAULTS.maxBend,
      settle: parseFloat(h.dataset.wavySettle) || WAVY_DEFAULTS.settle,
      freqStep: parseFloat(h.dataset.wavyFreq) || WAVY_DEFAULTS.freqStep
    };
    initWavyUnderline(h, cfg);
  });
}
function initWavyUnderline(heading, cfg = WAVY_DEFAULTS) {
  const wrap3 = heading.closest(".wavy-block") || heading.parentElement;
  if (!wrap3) return;
  const line = wrap3.querySelector(".wavy-block__underline");
  const hit = (line == null ? void 0 : line.querySelector(".wavy-block__hit")) || line;
  const svg = line == null ? void 0 : line.querySelector("svg");
  const path = (svg == null ? void 0 : svg.querySelector(".wavy-block__path")) || (svg == null ? void 0 : svg.querySelector("path"));
  if (!line || !svg || !path) return;
  let progress = 0, x = 0.5, reqId = null, time = Math.PI / 2;
  const clamp3 = (v, min, max) => Math.max(min, Math.min(max, v));
  setPath(progress);
  const ro = new ResizeObserver(() => setPath(progress));
  ro.observe(line);
  window.addEventListener("resize", () => setPath(progress), { passive: true });
  hit.addEventListener("pointerenter", onEnter, { passive: true });
  ...
  function onMove(e) {
    ...
    const clientX = e.clientX ?? rect.left + rect.width * x;
    const movementY = e.movementY ?? 0;
    const left = rect.left;
    const width = rect.width || 1;
    x = clamp3((clientX - left) / width, 0, 1);
    progress += movementY * cfg.sensitivity;
    setPath(progress);
  }
  function onLeave() {
    animateOut();
  ...
  function setPath(p) {
    const widthPx = line.getBoundingClientRect().width || 1;
    svg.setAttribute("viewBox", `0 0 ${widthPx} 100`);
    const cx = widthPx * x;
    const bend = clamp3(p * cfg.strength, -cfg.maxBend, cfg.maxBend);
    const cy = 50 + bend;
    path.setAttribute("d", `M0 50 Q${cx} ${cy}, ${widthPx} 50`);
  }
  function lerp(a, b, t) {
    return a * (1 - t) + b * t;
  }
  function animateOut() {
    ...
    time += cfg.freqStep;
    setPath(newProgress);
    progress = lerp(progress, 0, cfg.settle);
...
if (!isTouchDevice) {
  document.querySelectorAll(".hv-effect").forEach((element) => {
...
function fadeInAboutDecr() {
  gsapWithCSS.set(".about__decr", { autoAlpha: 0 });
  gsapWithCSS.to(".about__decr", {
    autoAlpha: 1,
    duration: 0.8,
    ease: "power2.out",
    scrollTrigger: {
...
window.addEventListener("load", () => {
  ...
  if ((_a = document.fonts) == null ? void 0 : _a.ready) {
    document.fonts.ready.then(() => {
      fadeInAboutDecr();
      if (typeof ScrollTrigger$1 !== "undefined") ScrollTrigger$1.refresh();
    });
  } else {
    fadeInAboutDecr();
  }
});
function fadeInOnScrollFooter() {
  const root = document.querySelector(".footer");
  if (!root) return;
  const topLevelBlocks = root.querySelectorAll(`
	.footer__offset--right > .footer__wavy,
	.footer__offset--right > .footer__info,
	.footer__offset--right > .wavy-block
  `);
  gsapWithCSS.set(topLevelBlocks, { autoAlpha: 0 });
  ...
  const infoSections = root.querySelectorAll(".footer__info .info__section");
  infoSections.forEach((sec) => {
    const kids = [
      sec.querySelector(".info__title"),
      ...sec.querySelectorAll("p, .info__list li, .info__email")
    ].filter(Boolean);
    if (!kids.length) return;
    gsapWithCSS.set(kids, { autoAlpha: 0 });
    gsapWithCSS.to(kids, {
      autoAlpha: 1,
      duration: 0.5,
      ease: "power2.out",
